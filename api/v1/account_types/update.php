...
if (empty($data['id']) || empty($data['name'])) {
    ...
    echo json_encode(["message" => "'id' and 'name' are required.", "response" => "error"]);
    exit();
}
$stmt = $db->prepare('UPDATE account_type SET name = ?, description = ? WHERE id = ?');
$stmt->execute([$data['name'], $data['description'] ?? null, $data['id']]);
